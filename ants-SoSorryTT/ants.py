...
def random_or_none(x):
    """ This function return random elementof list x or return None if x is empty."""
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        location = self.place
        index = 0
        while location.entrance != None or location != beehive:
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        firebees = self.place.bees.copy()
        # Ant.reduce_health(self, amount)
        for bee in firebees:
            self.place.bees[self.place.bees.index(bee)].reduce_health(amount)
        if amount >= self.health:
            for bee in self.place.bees.copy():
                self.place.bees[self.place.bees.index(bee)].reduce_health(self.damage)
        Ant.reduce_health(self, amount)
        # END Problem 5
...
class TankAnt(BodyguardAnt):
    ...
    def action(self, gamestate):
        ...
        # self.bees.reduce_health(self.damage)
        firebees = self.place.bees.copy()
        for bee in firebees:
            self.place.bees[self.place.bees.index(bee)].reduce_health(self.damage)
# END Problem 9
